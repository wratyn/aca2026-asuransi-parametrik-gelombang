...
for (z, k, p) in SEL:
    ...
    P_th = per_tahun(P)
    for sken, prem in (("A ketat", pr["bruto_A ketat"]), ("B ramah", pr["bruto_B ramah"])):
        for sub in (0.0, 0.5, 0.8):
...
D = pd.DataFrame(rows); D.to_csv(AKAR/"output/tables/dampak_per_sel.csv", index=False)
...
Rg = pd.DataFrame(ring); Rg.to_csv(AKAR/"output/tables/dampak_ringkas.csv", index=False)

sub_rows = []
for sken in ("A ketat","B ramah"):
    for sub in (0.0, 0.5, 0.8):
        tot = 0.0; n = 0
        for (z,k,_) in SEL:
            pr = PR[(PR.zona==z)&(PR.tier==k)].iloc[0]
...
SB = pd.DataFrame(sub_rows); SB.to_csv(AKAR/"output/tables/subsidi.csv", index=False)
